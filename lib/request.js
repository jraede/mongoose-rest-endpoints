...
module.exports = Request = (function() {
  ...
  Request.prototype.$post = function(req, res) {
    ...
    this.$$runHook('pre_filter', 'post', req, req.body).then(function(data) {
      ...
      return _this.$$runHook('pre_save', 'post', req, model).then(function(model) {
  ...
  Request.prototype.$$doBulkPostForSingle = function(obj, req) {
    var deferred,
      _this = this;
    deferred = Q.defer();
    this.$$runHook('pre_filter', 'bulkpost', req, obj).then(function(data) {
      var model;
      log('Successfuly ran pre_filter hook: ', JSON.stringify(data));
      model = new _this.$$modelClass(data);
      log('Saving normally (no cascade allowed on bulkpost)');
      return _this.$$runHook('pre_save', 'bulkpost', req, model).then(function(model) {
        return model.save(function(err, model) {
          if (err) {
            log('ERROR:'.red, 'Save failed:', err.message);
        ...
        log('ERROR:'.red, 'Error running pre_save hook: ', err.message);
        return _this.$$runHook('pre_response_error', 'bulkpost', req, httperror.forge(err.message, err.code != null ? err.code : 500)).then(function(err) {
          return deferred.reject(err);
        }, function(err) {
          return deferred.reject(err);
        });
