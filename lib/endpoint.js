...
Endpoint = (function() {
  ...
  Endpoint.prototype.handleRelationArray = function(req, prop, config) {
    ...
    if (this.postData[prop] instanceof Array) {
      ...
      for (childSchemaProp in _ref) {
        ...
        if ((childSchemaConfig.options.type != null) && childSchemaConfig.options.type === mongoose.Schema.Types.ObjectId && (childSchemaConfig.options.ref != null) && childSchemaConfig.options.ref.toLowerCase() === this.modelClass.modelName.toLowerCase()) {
  ...
  Endpoint.prototype.handleRelationObject = function(req, prop, config) {
    var child, childSchema, childSchemaConfig, childSchemaProp, deferred, parentProp, replacementProp, shouldBeSaving, subdoc, _ref,
      _this = this;
    deferred = Q.defer();
    ...
    for (childSchemaProp in _ref) {
      childSchemaConfig = _ref[childSchemaProp];
      if ((childSchemaConfig.type != null) && childSchemaConfig.type === mongoose.Schema.Types.ObjectId && (childSchemaConfig.ref != null) && childSchemaConfig.ref.toLowerCase() === this.modelClass.modelName.toLowerCase()) {
        parentProp = childSchemaProp;
        break;
      }
    }
    shouldBeSaving = _.keys(this.allowRelations).indexOf(prop) >= 0 ? true : false;
    child = this.postData[prop];
    if (typeof child === 'string') {
      replacementProp = child;
      this.replacePropBeforeResponse[prop] = child;
      deferred.resolve();
    } else {
      child = this.filterData(req, 'save', child, true);
      if (child._id) {
  ...
  Endpoint.prototype.handleRelations = function(req, data) {
    ...
    for (prop in _ref) {
      ...
      if (prop.substr(0, 1) === '_' && prop !== '_id' && prop !== '__v') {
        if (config instanceof Array && config[0].type === mongoose.Schema.Types.ObjectId && (data[prop] != null) && data[prop].length) {
          totalDeferreds.push(this.handleRelationArray(req, prop, config));
        } else if ((config.type != null) && (config.ref != null) && config.type === mongoose.Schema.Types.ObjectId && data[prop]) {
  ...
  Endpoint.prototype.get = function(req) {
    var data, deferred, err, filter, id, pop, query, _i, _len, _ref,
      _this = this;
    deferred = Q.defer();
    id = req.params.id;
    filter = {
    ...
    data = this.filterData(req, 'fetch', filter);
    if (!id) {
  ...
  Endpoint.prototype.put = function(req) {
    ...
    if (!id) {
      ...
      filter = this.filterData(req, 'fetch', filter);
