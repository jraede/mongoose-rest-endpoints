...
module.exports = Request = (function() {
      ...
      this.$$runHook('pre_filter', 'fetch', req, {}).then(function(filter) {
        ...
        return query.exec(function(err, model) {
          ...
          if (!model) {
            ...
            return _this.$$runHook('post_retrieve', 'fetch', req, model).then(function(model) {
      ...
      if (_this.$$endpoint.options.pagination) {
        ...
        return _this.$$modelClass.count(filter, function(err, count) {
          ...
          if (err) {
            ...
            if (_this.$$endpoint.options.limitFields != null) {
              query.select(_this.$$endpoint.options.limitFields.join(' '));
            }
            return query.sort(config.sortField).skip((config.page - 1) * config.perPage).limit(config.perPage).exec(function(err, collection) {
              if (err) {
                ...
                return _this.$$runHook('post_retrieve', 'list', req, collection).then(function(collection) {
                  var f, final, _i, _len;
                  final = [];
  ...
  Request.prototype.$post = function(req, res) {
    ...
    model = new this.$$modelClass(req.body);
    this.$$runHook('pre_save', 'post', req, model).then(function(model) {
  ...
  Request.prototype.$$doBulkPostForSingle = function(obj, req) {
    ...
    this.$$runHook('pre_save', 'bulkpost', req, model).then(function(data) {
      log('Successfuly ran pre_save hook: ', JSON.stringify(data));
      log('Saving normally (no cascade allowed on bulkpost)');
      return model.save(function(err, model) {
        if (err) {
          log('ERROR:'.red, 'Save failed:', err.message);
          return _this.$$runHook('pre_response_error', 'bulkpost', req, httperror.forge(err, 400)).then(function(err) {
            return deferred.reject(err);
          }, function(err) {
            return deferred.reject(err);
          });
      ...
      log('ERROR:'.red, 'Error running pre_filter hook: ', err.message);
      return _this.$$runHook('pre_response_error', 'bulkpost', req, httperror.forge(err, err.code != null ? err.code : 500)).then(function(err) {
        return deferred.reject(err);
      }, function(err) {
        return deferred.reject(err);
      });
    ...
    if (!(req.body instanceof Array)) {
      ...
      Q.allSettled(promises).then(function(results) {
        ...
        if (resolvedCount && !rejectedCount) {
          ...
          return _this.$$runHook('pre_response', 'bulkpost', req, results).then(function(results) {
            return deferred.resolve();
          }, function(err) {
            return deferred.reject(err);
          });
        ...
        return query.exec(function(err, model) {
          ...
          if (!model) {
            ...
            return _this.$$runHook('post_retrieve', 'put', req, model).then(function(model) {
              ...
              model.set(data);
              return _this.$$runHook('pre_save', 'put', req, model).then(function(model) {
                if (_this.$$endpoint.options.cascade != null) {
